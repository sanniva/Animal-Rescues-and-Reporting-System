...
// Report Detail Modal Component with Evidence, Completion Notes, and TRACKING MAP
const ReportDetailModal: React.FC<{
  report: RescueReport | null;
  isOpen: boolean;
  onClose: () => void;
  ...
  completionNotes = []
    ...
    useEffect(() => {
      if (report?.task_id && isOpen) {
        ...
        if (autoRefresh) {
          ...
          }, 5000); // Refresh every 5 seconds for smoother updates
    ...
    const fetchTrackingData = async (taskId: number) => {
      try {
        ...
        if (success && trackingData && trackingData.length > 0) {
          console.log('Tracking points found:', trackingData.length);
          ...
          console.log('No tracking points found');
          setTrackingPoints([]);
          setTrackingStats({
            pointCount: 0,
            startTime: null,
    ...
    const isInProgress = report.status_id === 3;
    const isCompleted = report.status_id === 4;
    const showTracking = !!report.task_id && (isInProgress || isCompleted); // Show for in-progress or completed

    const positions: [number, number][] = trackingPoints.map(point => [
      point.latitude,
      point.longitude
    ]);
                ...
                {showTracking && (
                  <div className="reports-info-card tracking-card">
                    <div className="reports-card-header beige">
                      <div className="reports-header-row">
                        <h4>
                          <span className="tracking-icon">🗺️</span> Live Tracking
                        </h4>
                      ...
                      <div className="reports-card-content">
                        {loadingTracking ? (
                          ...
                          <div className="tracking-container">
                            {/* Live Status Badge - Clean and simple */}
                            <div className="tracking-live-status" style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: '8px',
                              marginBottom: '16px'
                              ...
                              <div style={{
                                ...
                                height: '10px',
                                borderRadius: '50%',
                                backgroundColor: trackingStats.isLive ? '#4caf50' : '#9e9e9e',
                                boxShadow: trackingStats.isLive ? '0 0 8px #4caf50' : 'none'
                              }} />
                              <span style={{
                                fontSize: '0.85rem',
                                fontWeight: 500,
                                color: trackingStats.isLive ? '#2e7d32' : '#757575'
                              }}>
                                {trackingStats.isLive ? 'Ranger is active' : 'Ranger offline'}
                              </span>
                              {trackingStats.isLive && (
                                <span style={{
                                  fontSize: '0.7rem',
                                  color: '#666',
                                  marginLeft: 'auto'
                                }}>
                                  Last seen: {trackingStats.lastSeen}
                                </span>
                              )}
                            </div>
                            ...
                            <div style={{
                              ...
                              <div style={{
                                ...
                                <div style={{ fontSize: '0.7rem', color: '#666', marginBottom: '4px' }}>Points</div>
                                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: '#2D5A27' }}>
                                  {trackingStats.pointCount}
                                </div>
                              </div>
                              <div style={{
                                background: '#f5f5f5',
                                ...
                                <div style={{ fontSize: '0.7rem', color: '#666', marginBottom: '4px' }}>Distance</div>
                                <div style={{ fontSize: '1.1rem', fontWeight: 600, color: '#2D5A27' }}>
                                  {trackingStats.distance} km
                                </div>
                              </div>
                              <div style={{
                                background: '#f5f5f5',
                                ...
                                <div style={{ fontSize: '0.7rem', color: '#666', marginBottom: '4px' }}>Started</div>
                                <div style={{ fontSize: '0.9rem', fontWeight: 500 }}>
                                  {trackingStats.startTime || 'N/A'}
                                </div>
                              </div>
                              <div style={{
                                background: '#f5f5f5',
                                ...
                                <div style={{ fontSize: '0.7rem', color: '#666', marginBottom: '4px' }}>Last update</div>
                                <div style={{ fontSize: '0.9rem', fontWeight: 500 }}>
                                  {trackingStats.lastSeen || 'N/A'}
                                </div>
                              </div>
                            </div>

                            {/* Interactive Map */}
                            <div className="tracking-map-container" style={{ 
                              height: '300px', 
                              marginBottom: '16px', 
                              borderRadius: '8px', 
                              overflow: 'hidden',
                              ...
                              {positions.length > 0 && (
                                <MapContainer
                                  key={mapKey}
                                  center={positions[positions.length - 1]} // Center on last position
                                  zoom={15}
                                ...
                                >
                                  ...
                                  <Marker
                                    ...
                                    icon={trackingStats.isLive ? liveIcon : endIcon}
                                  >
                                    <Popup>
                                      <strong>{trackingStats.isLive ? 'Current' : 'Last seen'}</strong><br />
                            ...
                            {trackingStats.pendingPoints > 0 && (
                              <div style={{
                                ...
                                <span>⏳</span>
                                <span>{trackingStats.pendingPoints} points pending sync</span>
                              </div>
                            )}
                          </div>
                        ) : (
                          <div style={{
                            ...
                            <button
                              className="reports-btn primary small"
                              onClick={() => report.task_id && fetchTrackingData(report.task_id)}
                            >
                              Refresh
                            </button>
                          </div>
                      ...
                      <div className="reports-card-content" style={{ padding: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <span style={{ fontSize: '0.85rem' }}>
                            {trackingStats.isLive ? 'Live' : 'Last seen'}: {trackingStats.lastSeen || 'N/A'}
                          </span>
                        </div>
                      </div>
                    )}
...
const RescueReports: React.FC = () => {
  ...
  // State for evidence and completion notes
  const [taskEvidence, setTaskEvidence] = useState<{ [key: number]: TaskProof[] }>({});
  const [taskCompletionNotes, setTaskCompletionNotes] = useState<{ [key: number]: CompletionNote[] }>({});
