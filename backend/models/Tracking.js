...
class Tracking {
  ...
  static async createTable() {
    ...
    try {
      ...
      console.error('❌ Error creating task tracking table:', error.message);
  ...
  // Get tracking points for a task
  static async getTaskTrackingPoints(taskId, limit = 1000) {
    const sql = `
      SELECT 
        tt.tracking_id,
  ...
  static async updateSyncedStatus(trackingIds) {
    if (!trackingIds || trackingIds.length === 0) return 0;
