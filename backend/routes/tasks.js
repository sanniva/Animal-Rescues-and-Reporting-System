...
router.get('/:taskId/evidence', verifyToken, async (req, res) => {
  ...
  try {
    // Check if task exists and user has access (assigned volunteer or admin)
    let taskAccess = false;
    
    if (req.user.role_id === 3) {
      taskAccess = true; // Admin can view any evidence
    } else {
      const [taskCheck] = await pool.execute(
        'SELECT task_id FROM tasks WHERE task_id = ? AND assigned_to_user_id = ? AND is_deleted = 0',
        [taskId, userId]
      );
      taskAccess = taskCheck.length > 0;
    }
    
    if (!taskAccess) {
      return res.status(403).json({
        success: false,
        message: 'Forbidden: You can only view evidence for your assigned tasks'
      });
    }
...
// =====================================================
// GET COMPLETION NOTES FOR A TASK
// =====================================================
router.get('/:taskId/completion-notes', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role_id === 3) {
      taskAccess = true; // Admin can view any notes
    } else {
      const [taskCheck] = await pool.execute(
        'SELECT task_id FROM tasks WHERE task_id = ? AND assigned_to_user_id = ? AND is_deleted = 0',
        [taskId, userId]
    ...
    if (!taskAccess) {
      return res.status(403).json({
        success: false,
        message: 'Forbidden: You can only view notes for your assigned tasks'
      });
    }
    
    // Get all completion notes for this task
    const [notes] = await pool.execute(`
      SELECT 
        note_id,
        task_id,
...
// =====================================================
// GET TASK DETAILS WITH ALL RELATED DATA (for volunteers)
// =====================================================
router.get('/task/:taskId/full-details', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role_id === 3) {
      ...
      taskAccess = taskCheck.length > 0;
      if (taskAccess) {
        reportId = taskCheck[0].report_id;
      }
    } else {
      const [taskCheck] = await pool.execute(
        'SELECT task_id, report_id FROM tasks WHERE task_id = ? AND assigned_to_user_id = ? AND is_deleted = 0',
    ...
    // Get completion notes for this task
