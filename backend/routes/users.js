...
const storage = new CloudinaryStorage({
  ...
  params: {
    folder: 'animal-rescue/profile-images',
...
router.post('/:id/profile-image', verifyToken, upload.single('profile_image'), async (req, res) => {
  ...
  try {
    ...
    const oldUrl = userCheck[0].profile_image_url;
    if (oldUrl) {
...
router.delete('/:id/profile-image', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const currentUrl = userCheck[0].profile_image_url;
    if (currentUrl) {
...
router.delete('/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (profileImageUrl) {
      ...
      if (publicId) {
        await cloudinary.uploader.destroy(publicId).catch(err => {
          console.error('Error deleting profile image from Cloudinary on user delete:', err);
        });
      }
    }
